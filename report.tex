...
Naturally, it would be desirable to enable ordinary programmers to use such concepts, in order to reduce the time taken doing repetitive implementations for specific datatypes and reduce the number of bugs.
However, the state of the current literature is that it is often written targeting academics using abstract concepts, extensive type theory and complex features like dependent types.
For example, it is not uncommon in the literature to introduce many syntactic short cuts\,\autocite{Chapman:2010:GAL:1863543.1863547,mcbride2010ornamental} which makes it easier to write complex expressions, but at the cost of making it harder for the reader to immediately understand the underlying concepts.
...
I had started by analysing how a typical datatype is structured, and then showing how to convert datatypes to descriptions start with the simple \tttype{()} type, then progressing towards more complex indexed types such as \tttype{Vec}~\ttvar{a}~\ttvar{n}.
After providing an intuition on how to build descriptions for commonly used datatypes, I started adding some of the more complex concepts such as how to provide an informative encoding of constructor tags and how to synthesise actual types from provided descriptions.
...
This is especially troublesome when dealing with type errors because dependent type systems normalise while elaborating, which results in complex type errors with fully expanded terms even though aliases might have been used.
It is therefore important to provide a way to present understandable error messages in the future, perhaps by partially collapsing some of the erroneous terms when possible and using techniques such as error reflection\,\autocite{christiansen2014reflect}.
Another challenge it might be hard to understand the complete encoding of descriptions at first, but the complete encoding is required to do generic programming on all datatypes by the language.
It might be beneficial if different ``views'' could be presented of the description datatype, making it easier to understand essential features but perhaps at the cost of limiting the number of datatypes supported.
For example, one might provide a view that makes it easier to make generic algorithms on ordinary Haskell-like datatypes, by eliding details used for describing indexed types.
...
In the end, while generic programming may not yet be ready for main stream use; the provided tutorial should make it easier for interested programmers to learn.
Without extensive use and feedback from these first-movers, it can be hard to end up with a mature framework.
...
Described types provides a way to make algorithms which can be used for generically deriving type classes and describing other general properties.
The literature however is often scarce on presenting examples in a language that use practical features, such as type classes or tactics, like Idris.

Therefore, I aimed to implement a small collection of examples having commonly used operations in Chapter~\ref{cha:PracticalExamples}, both in terms of operations that form the base of type classes like \tttype{Show} and \tttype{DecEq} but also
functions that described purely generic properties like \ttdec{is}.
These example were written using an open world assumption, and therefore needed to interact with the type class resolution mechanism in order to retrieve the necessary constraints.
Some of these examples even used the built-in tactics mechanism to make it easier to use functions these generic functions, for example \ttdec{is} used proof search to find the correct \tttype{Tag} value given a particular constructors name.
This combination of practical features combined with dependent types, make it simpler for library implementers to provide implementations of generic operations with easy to use interfaces without requiring modifications to the compiler.
...
Additionally, while the presented description datatypes in Chapter~\ref{cha:GenericProgramming} and Chapter~\ref{cha:LevitatingIdris} were able to handle many kinds of datatypes, they could not handle all kinds.
In order to make generic programming using described types a viable approach to avoid boilerplate in all cases, additional work would be required to support for complex type families like the inductive-inductive and inductive-recursive ones.
...
Generic algorithms wouldn't be of great practical use, if they were orders of magnitude slower than their hand-written counterparts.
Therefore, it is essential that the generic algorithms written get optimised, and the added overhead eliminated such that they truly are a viable option to use.
...
This was the rationale for one of the important aims of this project, to investigate how it was possible to optimise algorithms dependent on described types.
Chapter~\ref{cha:OptimizingIdrisforFlight} presented an analysis on what the overhead added by encoding datatypes using descriptions, and using that analysis presented two techniques to eliminate the added overhead.
...
Since only the design have been sketched out for the presented algorithm, there are still things that are left to future work.
One of these things would be do provide an implementation of the algorithm, and then do some benchmarking to compare the performance and code size of optimised generic programs compared to hand-written ones.
Another thing would be to investigate what types of analyses are effective and if there are any heurestics that could be used to further improve performance.
For example, a static finitely enumerable parameter should probably not split if there is no dependent calculation.
Finally, it could be interesting to investigate how such algorithm could be extended to be polymorphic, since polymorphism in dependent type system is more loosely defined than in regular type systems.
Without polymorphic specialisation, it is not possible to specialise things like nested datatypes since there it would require creating an infinite series of datatypes (one for each nesting of parameters), similarly to the case of indices.
